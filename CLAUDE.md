...
- **Thesis-refresh workflow** (when the submodule snapshot bumps): in `thesis-src/` check out the new commit, drop in a refreshed `data/04_thesis_final.csv` if the run set changed and regenerate the lean file with `node backend/scripts/make-lean-csv.mjs`, regenerate the landing-page tour with `node backend/scripts/generate-tour.mjs` (answers and PDF anchors go stale with the snapshot; the refreshed PDF must come with its `report.toc`/`report.aux` or deep links degrade — startup logs a warning), regenerate the figure gallery with `node backend/scripts/figures-manifest.mjs` (re-copies + re-rasterizes the figures into `web/public/figures/` and merges `web/public/figures.json` from the new `.tex`/`report.aux`; it needs poppler `pdftoppm` on PATH or under `%LOCALAPPDATA%`/MiKTeX, else set `FIG_PDFTOPPM`; **author-written `title`/`description`/`models`/`wandbUrl` are preserved, only source-derived fields refresh**), re-run the §9 acceptance checks under `docker compose up`, then commit the bumped submodule pointer (+ both CSVs + `web/src/tour.json` + `web/public/figures.json` + `web/public/figures/`) as `Refresh thesis snapshot to <short-sha>` and deploy. See `Initial_plan.md` §3.1.
- **Figure gallery** (built 2026-06-13): a public `/figures` page (`web/src/components/Gallery.tsx`) over a hand-maintained manifest `web/public/figures.json` (type + loader + citation matcher in `web/src/figures.ts`), with committed PNG (display) + source-PDF assets under `web/public/figures/`. The manifest is the source of truth and is **hand-edited** — `backend/scripts/figures-manifest.mjs` only seeds it from the pinned thesis (caption→description, page, figure file) and never clobbers author-written prose. Each card shows what the figure is, the model(s) behind it (`models`), and an author-curated W&B link (`wandbUrl`). In chat, a `[thesis: fig:…]` chip that matches a manifest entry opens the actual plot inline (`web/src/components/FigurePanel.tsx`, sibling of `ThesisPanel`) instead of the raw PDF page. Captions render through a shared `web/src/components/Markdown.tsx` (remark-math + KaTeX). Assets serve statically (no backend route; Vite copies `public/` → `dist/`, so no dev-proxy entry either). Code citations were intentionally left as GitHub link-outs (`codeCitationUrl`).
